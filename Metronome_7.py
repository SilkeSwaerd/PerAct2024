...
participant_dialog.addField("Have you received more than 3 years of formal musical education?", choices=["Yes", "No"])
...
# Consent Dialog
consent_text = visual.TextStim(win, text="If you are sensitive to rapid flashing lights, this experiment might bother you, otherwise it poses no risks.\n\nPress 'space' to consent or 'escape' to exit.",
...
def pulsating_circle(clock, metronome, duration, include_metronome=False):
    ...
    while clock.getTime() < end_time:
        ...
        # Control the fixation cross for the "Metronome" condition
        if task_type == "Metronome":
            fixation.draw()  # Draw the fixation cross once at the beginning of each loop iteration
        ...
        # Refresh the window after drawing the circle
        win.flip()
